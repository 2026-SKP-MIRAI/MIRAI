...
각 커밋을 카테고리로 분류한 뒤, 기술적 메시지를 팀원이 읽기 좋은 문장으로 변환한다.
...
- 한국어 톤 유지 (기존 CHANGELOG 스타일 따름)
- 관련 커밋 여러 개가 같은 기능이면 하나의 항목으로 합친다
- 내부 리팩토링·테스트 픽스처 커밋은 상위 기능 항목에 포함시킨다
...
새로운 주차 섹션을 기존 CHANGELOG.md 첫 번째 `---` 구분선 바로 아래에 **prepend** 한다.
- 이미 같은 주차 헤더가 있으면 해당 섹션에 항목을 **병합**한다 (중복 제거)
- CHANGELOG.md가 없으면 헤더와 함께 새로 생성한다
...
scope가 단일인 경우 해당 CHANGELOG.md 업데이트된 내용 전체를 출력한다.

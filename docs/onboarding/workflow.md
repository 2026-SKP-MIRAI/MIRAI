...
git worktree add .worktree/feat-15 -b feat/15-pdf-upload    # 기능 구현
git worktree add .worktree/fix-23  -b fix/23-parser-error   # 버그 수정
git worktree add .worktree/docs-31 -b docs/31-interface-doc # 문서 작업
```
...
브랜치 규칙: `타입/이슈번호-짧은설명`
경로 규칙: `.worktree/<이슈번호>` (레포 루트 내부, `.gitignore`로 추적 제외)
...
```bash
# 레포 루트로 복귀 후 실행
git worktree remove .worktree/feat-15

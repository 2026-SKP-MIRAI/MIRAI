...
> 서비스: Fint (Find Interview — "면접, 재밌게 준비하자")
...
| 크레딧 획득 | "면접 한 번에 크레딧 N개, 쌓이다 보면 빠르게 성장" [미확정] |
...
| 크레딧 부족 | "크레딧이 조금 부족해요. 면접 연습을 더 하면 쌓을 수 있어요!" |
...
- Duolingo 올빼미처럼 Fint를 대표하는 취준생 아이콘화 마스코트 도입 검토 중
...
| 크레딧 아이콘 | 황금색 동전 심볼 (`--coin` 컬러 적용, 게임 UI 레퍼런스) |
...
| 사용처 | 온보딩, 빈 상태(empty state), 완료 화면, 크레딧 획득 애니메이션 |
...
<div className="flex items-center gap-2 rounded-2xl bg-coin/10 px-4 py-3 shadow-md">
  ...
  <div>
    <p className="text-sm font-semibold text-coin">크레딧 N개 획득! {/* [미확정] */}</p>
...
Duolingo는 올빼미 "Duo"를 마스코트로 사용하며 강력한 브랜드 인지도와 리텐션 넛지(스트릭 깨면 Duo가 울음)를 만들어냈다. Fint도 면접 코치 역할의 캐릭터를 도입할 경우 유사 효과를 기대할 수 있다.
...
`globals.css`에 옵션 B CSS 토큰을 적용한다. 아래 §2 옵션 B 토큰 참조.
...
| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-03-17 | 키 컬러 확정 | 옵션 B(틸 #0D9488) 최종 선정 — 3종 목업 비교 후 결정 |

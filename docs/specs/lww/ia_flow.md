...
│   │   ├── 점수 리포트
│   │   ├── 실행형 피드백
│   │   ├── 합격 예언 오브 (코인 소비, Phase 1 후반)
│   │   ├── 고득점 Q&A 패턴 열람 (코인 소비, Phase 1 후반)
│   │   └── 면접 후기 작성 CTA (코인 획득, Phase 1)
│   └── 면접 히스토리
│
├── 마켓 (Phase 2)
│   ├── 페르소나 목록
...
│   ├── 아차 포인트 (코인 소비)
│   ├── 회사 평가 (코인 소비)
│   ├── 연봉 통계 열람 (코인 소비, Phase 2)
│   ├── 이직 타이밍 분석 (코인 소비, 현직자용, Phase 2)
│   ├── 동료 스카우트 후기 (코인 소비, 현직자용, Phase 2)
│   ├── 직무 MBTI 카드 (코인 소비, 바이럴 콘텐츠, Phase 2)
│   ├── 오늘의 면접 운세 (코인 소비, Phase 2)
│   └── 코인 충전
│
├── 커뮤니티 (Phase 2)
│   ├── 취준생 게시판
...
| 단계 | 이탈 원인 | 해결책 |
|------|-----------|--------|
| 현직자 선택 | 혜택 불명확 | "내 경험이 취준생에게 실제 도움이 됐대 — 재밌지 않아?" (재미·보람 강조, 코인은 2순위) |
| 회사 인증 | 번거로움 | 이메일 인증 우선 제공, 재직증명서 업로드 보조 옵션 |
| 직군·연차 설정 | 세분화 부담 | 대분류만 필수, 소분류 선택. 🪙 연봉·직군·연차 상세 입력 시 코인 지급 (Phase 2 활동 보상) |
...
### Task 4: 아차 포인트 코인으로 잠금 해제 (Phase 2)
...
| **Phase 1** | 자소서 업로드 + 코인 획득 + 스트릭·레벨업 | 개인화·리텐션 루프 검증 |
| **Phase 1 후반** | 합격 예언 오브 + 고득점 Q&A 패턴 열람 (코인 소비) | 리텐션 강화 (코인 소비처 오픈) |
| **Phase 2+3** | 홈 피드 + 커뮤니티 (취준생·현직자 게시판) + 마켓 (페르소나 목록·상세) + 쪽지 + 현직자 코인 소비처 (연봉 통계·이직 타이밍 분석·동료 스카우트 후기) + 직무 MBTI 카드 + 응원 폭탄 + 채용 매칭 탭 + 기업 직접 연결 기능 | 커뮤니티·네트워크 효과 + 수익화 (마감: 2026-04-01) |

...
> **미인증 계정 제한**: 이메일 인증 전까지 면접 결과 저장·코인 획득 불가. MVP 익명 세션과 동일 취급.
...
- [ ] 일일 획득 상한 설정 (N코인/일, 어뷰징 방지)
- [ ] AI 페르소나 계정의 코인 집계 제외
- [ ] 코인 내역 조회 API
...
> **보안 주의**: `/api/coins/earn`은 클라이언트가 직접 호출하면 어뷰징 가능. **코인 지급은 서버 이벤트(면접 완료, 리포트 조회)에서만 서버 사이드로 처리**해야 하며, 클라이언트에서 직접 호출하는 흐름은 금지.

**Phase 1 후반 코인 소비처 (별도 기능 명세):**
- [ ] 합격 예언 오브: 결과 리포트 화면에서 코인 소비 후 잠금 해제 (코인 수량 미확정)
- [ ] 고득점 Q&A 잠금 해제: 동일 질문 고득점 답변 열람 (코인 수량 미확정)
...
**CS 체계:**
- [ ] 코인 분쟁 신청 폼
- [ ] 결제 취소 처리 API (미사용 코인 7일 청약철회 [검토 중])
...
- [ ] 취준생이 코인으로 페르소나 면접 구매
- [ ] 코인 배분: 현직자 70%, 플랫폼 30% (자동 계산)
- [ ] 페르소나 평점 시스템 (5점 만점)
- [ ] 인기순·직군별 필터링
- [ ] 현직자 프로필 완성 시 코인 지급 (연봉·직군·연차 입력 완료, 수량 미확정)
...
**AC:**
- [ ] 익명 응원 폭탄: 취준생이 코인 소비 → 랜덤 취준생에게 익명 응원 발송
- [ ] 직무 MBTI 카드: 면접 기록 기반 성향 분석 결과 카드 생성 (공유 가능)
- [ ] 연봉 통계: 현직자 익명 연봉 데이터 기반 직군별 연봉 분포 열람 (코인 소비)
- [ ] 아차 포인트: 면접 중 놓친 핵심 키워드 분석 제공 (코인 소비)
- [ ] 고득점 Q&A: 동일 질문 고득점 답변 사례 열람 (코인 소비, Phase 1 후반 일부 오픈)

**API:**
```
POST /api/community/cheer-bomb   → { coins_deducted, targetUserId }
GET  /api/stats/mbti-card        → { mbtiType, traits[], shareUrl }
GET  /api/stats/salary?category= → { distribution[], median } (코인 소비)
GET  /api/interview/aha-points?sessionId= → { missedKeywords[] } (코인 소비)
```

### 2.5 운영 모니터링 대시보드

**지표:**
- 코인 순환 현황 (총 발행량 vs 소비량)
- 어뷰징 탐지 알림
...
- **코인 이체 금지:** API 레벨에서 유저 간 코인 전송 엔드포인트 없음
- **AI 페르소나 식별:** DB 레벨 `type: 'ai_persona'` — 코인 집계·지표에서 항상 제외
- **법적 고지:** 이용약관에 코인 현금화 불가, AI 혼재 플랫폼 명시
- **코인 지급 서버 사이드 전용:** `/api/coins/earn`은 서버 이벤트에서만 호출. 클라이언트 직접 호출 금지.

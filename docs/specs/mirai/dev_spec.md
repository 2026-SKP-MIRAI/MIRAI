...
**기능 정의:** "짧은 과제 → 즉각 피드백 → 재답변 → 피드백"의 의지적 연습 순환 구조. 답변하면 즉각 피드백을 제공하고, 재답변 한 번 후 그에 대한 피드백을 제공한다.

**흐름:** 답변 → 즉각 피드백 → 재답변 → 피드백
...
> `growthCurve`: 엔진은 항상 `null` 반환. 서비스(Next.js)가 Supabase 세션 이력에서 날짜별 총점·8축 점수 배열을 조립하여 클라이언트에 제공.
...
| **Phase 2.5** | 각 서비스 Supabase DB 연동 (기능 03·04 이후) | 세션 이력 영속화, growthCurve 집계 기반 마련 |

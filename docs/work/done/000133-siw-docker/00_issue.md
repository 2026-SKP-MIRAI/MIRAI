...
### 2026-03-18 — EC2 배포 검증 및 HEALTHCHECK 수정

**EC2 배포 결과**
- `workflow_dispatch`로 `refactor/000133-siw-docker` 브랜치 직접 배포 테스트
- `docker logs siw` → `No pending migrations to apply.` + Next.js 서버 기동 확인
- ALB DNS 접속 → HTML 응답 확인 (ALB 보안 그룹 80번 인바운드 규칙 수정 필요했음)

**HEALTHCHECK `unhealthy` 추가 수정**
- 원인: Alpine 컨테이너에서 `localhost`가 `::1`(IPv6)로 resolve → 서버는 IPv4(`0.0.0.0`)만 리스닝 → Connection refused
- 수정: `CMD wget -qO- http://localhost:3000/` → `http://127.0.0.1:3000/`
- 결과: `docker inspect siw` → `Status: healthy` 확인

...
### 2. EC2 인스턴스 생성 (siw)
- [ ] siw 서버 — t3a.micro 이상, Ubuntu 22.04, 보안 그룹: SSH(22), HTTP(80), HTTPS(443) 만 오픈 (3000은 ALB 보안 그룹에서만 허용)
...
> 000064에서 engine/lww용 ALB 2개가 이미 구성됨. siw용 ALB를 별도 생성하거나 기존 ALB에 타겟 그룹 추가.
- [ ] ALB 생성 (siw용) — Internet-facing, HTTPS 리스너(443) + HTTP→HTTPS 리다이렉트(80)
- [ ] 타겟 그룹 생성 — Instance 타입, 포트 3000, 헬스체크 경로 `/`
- [ ] EC2 인스턴스를 타겟 그룹에 등록
- [ ] ALB 보안 그룹 — HTTP(80), HTTPS(443) 인바운드 오픈
- [ ] EC2 보안 그룹 — ALB 보안 그룹에서 포트 3000 인바운드 허용 (퍼블릭 3000 직접 오픈 금지)
...
- [ ] **`SizeRestrictions_BODY` → Count로 변경** (이력서 PDF 업로드가 있으므로 필수. 000064에서 동일하게 처리함. 안 하면 PDF 업로드 WAF에서 차단됨)
- [ ] Web ACL을 ALB에 연결
...
- [ ] `SIW_EC2_HOST` — siw EC2 **Elastic IP** (재시작해도 IP 고정됨)
- [ ] `SIW_EC2_USER` — `ubuntu`

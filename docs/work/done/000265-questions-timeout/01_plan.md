...
올바른 체인: `engine(50s) < fetch(55s) < maxDuration(60s)`
...
def generate_questions(
    resume_text: str,
    *,
    timeout_seconds: float = 50.0,  # 30.0에서 변경
```

### 타임아웃 계층 역할
1. **engine(50s)**: LLM이 50s 안에 응답 없으면 에러 반환
2. **fetch abort(55s)**: 엔진이 정상 에러를 반환하면 55s 전에 수신. 엔진이 멈추면 55s에 abort

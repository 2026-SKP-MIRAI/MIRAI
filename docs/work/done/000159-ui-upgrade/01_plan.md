...
- [ ] `/login`, `/signup` 페이지 UI 고도화
- [ ] `/resume` 페이지 UI 고도화 — 업로드 폼, 질문 리스트, 면접 시작/서류 진단 분기 카드
- [ ] UploadForm 드래그&드롭 + 파일 크기(5MB 이하)·형식(PDF) 안내 텍스트 추가
- [ ] `/interview` 페이지 UI 고도화 — 채팅 UI, 답변 입력 폼, 리포트 생성 버튼
- [ ] `/report` 페이지 UI 고도화 — 총점, 8축 점수, 피드백 레이아웃
- [ ] `/diagnosis` 페이지 UI 고도화 — 항목별 점수, 강점·약점·개선 방향
- [ ] `/dashboard` 페이지 UI 고도화 (이슈 #157 이후 — 조건부 완료, 아래 참조)
- [ ] 전체 페이지 공통 색상·타이포그래피·간격 일관성 확보
- [ ] 로딩·에러·빈 상태 피드백 UI 일관성 확보
- [ ] 모바일 반응형 대응
- [ ] 기존 Vitest/E2E 회귀 없음 (UI 변경으로 인한 셀렉터 업데이트 포함)
- [ ] 해당 디렉토리 .ai.md 최신화
- [ ] 불변식 위반 없음

...
class TestValidateFollowupOverlap:
    def test_overlap_sufficient_no_regeneration(self):
        ...
        # embedding 두 벡터가 cosine_similarity >= 0.5 를 만들도록 동일 벡터 반환
        high_score_embeddings = [[1.0, 0.0], [1.0, 0.0]]  # score = 1.0
        get_embeddings_fn = MagicMock(return_value=(high_score_embeddings, None))
    ...
    def test_overlap_low_first_then_pass(self):
        ...
        # 1차: score 0.0 (직교), 2차: score 1.0 (동일)
        low_embeddings = [[1.0, 0.0], [0.0, 1.0]]   # cosine = 0.0
        high_embeddings = [[1.0, 0.0], [1.0, 0.0]]  # cosine = 1.0
        get_embeddings_fn = MagicMock(side_effect=[(low_embeddings, None), (high_embeddings, None)])
    ...
    def test_max_attempts_reached(self):
        ...
        # 항상 낮은 score
        low_embeddings = [[1.0, 0.0], [0.0, 1.0]]  # cosine = 0.0
        get_embeddings_fn = MagicMock(return_value=(low_embeddings, None))
    ...
    def test_embedding_failure_graceful(self):
        """get_embeddings_fn raises Exception → 초기 결과 반환, 예외 전파 없음."""
        initial = _make_response("question A")
        generate_fn = MagicMock()

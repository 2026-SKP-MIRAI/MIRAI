...
def test_generate_questions_success():
    with patch("app.services.llm_service.OpenAI", return_value=make_mock_client(VALID_RESPONSE)):
...
def test_generate_questions_invalid_json_raises_llm_error():
    with patch("app.services.llm_service.OpenAI", return_value=make_mock_client("not json")):
        with pytest.raises(LLMError):
            generate_questions("이력서")

def test_generate_questions_unknown_category_raises_llm_error():
    bad = '[{"category":"모르는카테고리","question":"질문?"}]' * 8
    with patch("app.services.llm_service.OpenAI", return_value=make_mock_client(bad)):
        with pytest.raises(LLMError):
            generate_questions("이력서")

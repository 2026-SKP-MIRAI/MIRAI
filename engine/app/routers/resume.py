...
from app.schemas import ParseResponse, QuestionsRequest, QuestionsResponse, Meta, ResumeFeedbackRequest, ResumeFeedbackResponse
...
async def parse_resume(request: Request, file: UploadFile | None = File(None)):
    logger.info("[resume/parse] 요청 수신: 파일명=%s, content_type=%s",
                file.filename if file else None, file.content_type if file else None)

    if file is None:
        logger.warning("[resume/parse] 파일 없음")
        raise ParseError("파일이 없습니다. PDF 파일을 업로드해 주세요.")

    # content_type 검증 (MIME 위장 방지) — 파일을 읽기 전에 검증
    content_type = file.content_type or ""
    if "pdf" not in content_type.lower():
        raise ParseError("PDF 파일만 업로드 가능합니다.")

    # Content-Length 기반 선제 크기 제한 (메모리 DoS 방지)
    # 헤더값은 multipart 전체 크기이므로 실제 파일 크기보다 약간 크다 — 1KB 여유를 둔다
    ...
    logger.info("[resume/parse] 파싱 완료: 텍스트 길이=%d", parsed.extracted_length)
...
async def create_questions(body: QuestionsRequest):
    ...
    questions = generate_questions(body.resumeText)
    ...
    return QuestionsResponse(
        questions=questions,
        meta=Meta(
            extractedLength=len(body.resumeText),
